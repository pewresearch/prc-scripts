/**
 * Block editor helpers (gap presets, nested block search).
 */
 ...
 * @return {Object|null}
 */
export function findBlock( blocks, wildcard, depth = 0 ) {
	if ( depth > 5 ) {
		return null;
	}
	...
	for ( const block of blocks ) {
		if (
			block.blockName &&
			new RegExp( wildcard.replace( '*', '.*' ) ).test( block.blockName )
		) {
			return block;
		}

		if ( block.innerBlocks && block.innerBlocks.length > 0 ) {
			const innerBlock = findBlock(
				block.innerBlocks,
				wildcard,
				depth + 1
			);
 ...
 *
 * @param {*}      attributes
 * @param {string|boolean} dimensionToReturn
 * @return {string}
 */
export function getBlockGapSupportValue(
	attributes,
	dimensionToReturn = false
) {
	let blockGap = attributes?.style?.spacing?.blockGap;
	if ( typeof blockGap === 'object' && false !== dimensionToReturn ) {
		const key = 'horizontal' === dimensionToReturn ? 'left' : 'top';
		blockGap = blockGap[ key ];
	}
	if ( typeof blockGap !== 'string' ) {
		return '';
	}
	return blockGap?.startsWith( 'var:preset|spacing|' )
		? `var(--wp--preset--${ blockGap
				.replace( 'var:preset|', '' )
				.replace( '|', '--' ) })`
		: blockGap;
}
